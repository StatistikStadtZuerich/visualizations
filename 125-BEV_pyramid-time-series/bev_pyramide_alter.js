(function(d3, sszvis, bev_pyramide_alterParams) {
  'use strict';
  ...
  var config = {
    ...
    dataPath: bev_pyramide_alterParams.data,
    // The title and description of this chart for visually impaired users
    title: bev_pyramide_alterParams.title,
    description: bev_pyramide_alterParams.description,
    ...
    fallback: false,
    targetElement: bev_pyramide_alterParams.id
  ...
  var actions = {
    ...
    selectYear: function(year) {
      ...
      state.maxValue = d3.max([d3.max(state.rawData, vAcc), 6000]);
  ...
  function isLeft(d) {
    return cAcc(d) === 'Frauen';
  }

}(d3, sszvis, bev_pyramide_alterParams));
